...
class __TwigTemplate_377c0a2155957a5784f4afd2970902389d4897c78c81b3c3a1449856a7ed291b extends \Twig\Template
{
...
<video data-setup=\"{}\" class=\"video-js vjs-default-skin\" preload=\"{{ player_attributes.preload }}\" {{ player_attributes.controls ? 'controls' : '' }} style=\"width:{{ player_attributes.width }}px;height:{{ player_attributes.height }}px;\" {{ player_attributes.autoplay ? 'autoplay' : '' }} {{ player_attributes.loop ? 'loop' : '' }} {{ player_attributes.muted ? 'muted' : '' }}>
  {% for user in items %}
    <source src=\"{{ user }}\"/>
  {% endfor %}
</video>", "modules/contrib/videojs/templates/videojs.html.twig", "/opt/lampp/htdocs/riboville-drupal-8/modules/contrib/videojs/templates/videojs.html.twig");
    }
}
